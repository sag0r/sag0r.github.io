(function ($) {
		...
		$.each(all_converter, function (e) {
			...
			$.each(tree, function (i, e) {
				...
				if (numHyphen < numHyphen_next) {
					...
					item = item + '<i class="bi bi-plus"></i>' + "\n";
	...
	$.fn.hummingbird.defaults = {
		SymbolPrefix: "bi",
		expandedSymbol: "bi-dash",
		collapsedSymbol: "bi-plus",
		collapseAll: true,
		checkboxes: "enabled",
		...
		$(dialog + " #" + search_button).on("click", function (e) {
			...
			tree.find('input:checkbox' + onlyEndNodes_Class).each(function () {
				if ($(this).parent().text().toUpperCase().includes(search_str.toUpperCase())) {
					...
					$(dialog + " #" + search_output).append('<li class="dropdown-item" id="drop_' + $(this).attr("id") + '"><a href="#">' + $(this).parent().text() + '</a></li>');
					num_search_results++;
				}
			});
